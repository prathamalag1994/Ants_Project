...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if (not self.place.ant) or (self.place.ant != None and self.place.ant.blocks_path == False):
            return False
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container == True and self.ant == None and other.container == False:  #ant.container possibility 
            return True
        else:
            return False
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        self.armor = 1
        ninja_place_bees = self.place.bees[:]
        for bees in ninja_place_bees:
            bees.reduce_armor(self.damage)
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        self.own = colony_queen
        self.original = ant_queen

    @property
    def bees(self):
        return self.own.bees + self.original.bees
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        self.imposter = False
        if QueenAnt.queen_placed == False:
            QueenAnt.queen_placed = True
        else:
            self.imposter = True
    ...
    def action(self, colony):
        ...
        if not self.imposter:
            ...
            while (current != None):
                if current.ant != None:
                    if not current.ant in self.ant_list:
                        current.ant.damage *= 2
                        if current.ant.container == True:
                            current.ant.ant.damage *= 2
                            self.ant_list += [current.ant.ant]
                        self.ant_list += [current.ant]
